...
// On ESP32-S3, ADC1 channels 0..9 are GPIO1..10.  The older ESP32
// register layout below does not exist on this target.
...
#else
// ESP32-S3 exposes only the low-speed LEDC channel output matrix.
static byte LEDCToMux[] = {
  LEDC_LS_SIG_OUT0_IDX,
  LEDC_LS_SIG_OUT1_IDX,
  LEDC_LS_SIG_OUT2_IDX,
...
#ifdef ADC_ATTEN_6db
  // GPIO36 receives the reduced L298P current-sense signal (up to 1.67 V measured).
  // 6 dB keeps the short-circuit peak in range and resolves low running current better.
  adc1_config_channel_atten(pinToADC1Channel(pin),ADC_ATTEN_6db);
#else
  adc1_config_channel_atten(pinToADC1Channel(pin),ADC_ATTEN_DB_6);
#endif
