...
void feedTheDog0(){
  // Timer-group register names differ on ESP32-S3.
#if defined(CONFIG_IDF_TARGET_ESP32S3)
  TIMERG0.wdtwprotect.wdt_wkey = TIMG_WDT_WKEY_V; // write enable
  TIMERG0.wdtfeed.wdt_feed = 1;                   // feed watchdog
  TIMERG0.wdtwprotect.wdt_wkey = 0;               // write protect
#else
  TIMERG0.wdt_wprotect = TIMG_WDT_WKEY_VALUE; // write enable
  TIMERG0.wdt_feed = 1;                       // feed watchdog
  TIMERG0.wdt_wprotect = 0;                   // write protect
#endif
  // TIMERG1 is deliberately not fed here.
}
